...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        vector<size_t> idx01 = Initializations::TwoWalls(Phase, 0, 1, Thickness, BC);
        EN.ThermalSurfaceCondition.push_back({ThermalCondition::Type::ConstantFlux, EN.HeatFlux, idx01[1], 0.0});
        if(Phase.Grid.OffsetX==0 and Phase.Grid.OffsetZ==0)
        {
            cout<<"PhaseField number ["<<idx01[0]<<"]= is created"<<endl;
        }

        double Lx              = SB.X0DistZone;
        double Ly              = Phase.Grid.TotalNy;
        double Lz              = 0.2*SB.PartDiameter/FL.Grid.dx;
        double DiameterNozzle  = 1.1*SB.PartDiameter/FL.Grid.dx;

        double NozzleX0 = Lx/2;
        double NozzleY0 = Ly;
        double NozzleZ0 = Phase.Grid.TotalNz/2 - DiameterNozzle/2.0;
        size_t idx2=Initializations::Rectangular(Phase,1,Lx,Ly,Lz,NozzleX0,NozzleY0, NozzleZ0, BC);
        EN.ThermalSurfaceCondition.push_back({ThermalCondition::Type::ConstantFlux, EN.HeatFlux, idx2, 0.0});
        if(Phase.Grid.OffsetX==0 and Phase.Grid.OffsetZ==0)
        {
            cout<<"PhaseField number ["<<idx2<<"]= is created"<<endl;
        }

        double NozzleXN = Lx/2;
        double NozzleYN = Ly;
        double NozzleZN = Phase.Grid.TotalNz/2 + DiameterNozzle/2.0;
        size_t idx3=Initializations::Rectangular(Phase,1,Lx,Ly,Lz,NozzleXN,NozzleYN, NozzleZN, BC);
        EN.ThermalSurfaceCondition.push_back({ThermalCondition::Type::ConstantFlux, EN.HeatFlux, idx3, 0.0});
        if(Phase.Grid.OffsetX==0 and Phase.Grid.OffsetZ==0)
        {
            cout<<"PhaseField number ["<<idx3<<"]= is created"<<endl;
        }
