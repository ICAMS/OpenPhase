...
int main(int argc, char *argv[])
{
    ...
    else
    {
       /* // Single liquid phase starting configuration
        ...
        Phi.FieldsProperties[idx1].State = AggregateStates::Liquid;
        ...
        Phi.FieldsProperties[idx2].State = AggregateStates::Solid;
        // Orienting the nucleus as desired
        EulerAngles locAngle({Pi/2.0, 0, Pi/8.0}, EulerConventions::XYZ);
        ...
        size_t idx1 = Initializations::Single(Phi, 1, BC);
        //size_t idx2 = Initializations::Sphere(Phi, 0, OPSettings.Dimensions.Nz/2 - 4, (OPSettings.Dimensions.Nx)/2,(OPSettings.Dimensions.Ny)/2,(OPSettings.Dimensions.Nz)/2, BC);
        size_t idx2 = Initializations::Ellipsoid(Phi, 0, OPSettings.Grid.Nx/2 - 5, OPSettings.Grid.Ny/2 - 5, OPSettings.Grid.Nz/2 - 5, (OPSettings.Grid.Nx)/2, (OPSettings.Grid.Ny)/2, (OPSettings.Grid.Nz)/2, BC);
    ...
    for(RTC.tStep = RTC.tStart; RTC.tStep <= RTC.nSteps; RTC.IncrementTimeStep())
    {
        ...
        {//  Write data in VTK format
            dG.WriteVTK(tStep, 1, 0);
            dG.WriteVTK(tStep, 2, 0);
        }*/
