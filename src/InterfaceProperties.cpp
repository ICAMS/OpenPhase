...
//    }
    ...
    MaxExtrapolationIterations = FileInterface::ReadParameterI(inp, moduleLocation, std::string("MaxExtrapolationIterations"), false, 6);
...
bool InterfaceProperties::ZerothOrderExtrapolation(const PhaseField& Phase)
{
    ...
    if (PF(i,j,k).wide_interface())
    {
        ...
        if (*beta > *alpha)
        {
            if (IP(i,j,k).get_energy(*alpha, *beta) == 0.0)
            {
                ...
                // zeroth order extrapolation and zeroth order weighting (compare to Riemann problem)
                if (Grid.dNx            ) {locE1 = IP(i+1,j,k)  .get_energy(*alpha, *beta); if (locE1 != 0.0) { sumEnergies += locE1; count++; }}
                if (Grid.dNx            ) {locE1 = IP(i-1,j,k)  .get_energy(*alpha, *beta); if (locE1 != 0.0) { sumEnergies += locE1; count++; }}
                if (Grid.dNy            ) {locE1 = IP(i,j+1,k)  .get_energy(*alpha, *beta); if (locE1 != 0.0) { sumEnergies += locE1; count++; }}
                if (Grid.dNy            ) {locE1 = IP(i,j-1,k)  .get_energy(*alpha, *beta); if (locE1 != 0.0) { sumEnergies += locE1; count++; }}
                if (Grid.dNz            ) {locE1 = IP(i,j,k+1)  .get_energy(*alpha, *beta); if (locE1 != 0.0) { sumEnergies += locE1; count++; }}
                if (Grid.dNz            ) {locE1 = IP(i,j,k-1)  .get_energy(*alpha, *beta); if (locE1 != 0.0) { sumEnergies += locE1; count++; }}
                if (Grid.dNx && Grid.dNy) {locE1 = IP(i-1,j-1,k).get_energy(*alpha, *beta); if (locE1 != 0.0) { sumEnergies += locE1; count++; }}
                if (Grid.dNx && Grid.dNy) {locE1 = IP(i+1,j-1,k).get_energy(*alpha, *beta); if (locE1 != 0.0) { sumEnergies += locE1; count++; }}
                if (Grid.dNx && Grid.dNy) {locE1 = IP(i-1,j+1,k).get_energy(*alpha, *beta); if (locE1 != 0.0) { sumEnergies += locE1; count++; }}
                if (Grid.dNx && Grid.dNy) {locE1 = IP(i+1,j+1,k).get_energy(*alpha, *beta); if (locE1 != 0.0) { sumEnergies += locE1; count++; }}
                if (Grid.dNx && Grid.dNz) {locE1 = IP(i-1,j,k-1).get_energy(*alpha, *beta); if (locE1 != 0.0) { sumEnergies += locE1; count++; }}
                if (Grid.dNx && Grid.dNz) {locE1 = IP(i+1,j,k-1).get_energy(*alpha, *beta); if (locE1 != 0.0) { sumEnergies += locE1; count++; }}
                if (Grid.dNx && Grid.dNz) {locE1 = IP(i-1,j,k+1).get_energy(*alpha, *beta); if (locE1 != 0.0) { sumEnergies += locE1; count++; }}
                if (Grid.dNx && Grid.dNz) {locE1 = IP(i+1,j,k+1).get_energy(*alpha, *beta); if (locE1 != 0.0) { sumEnergies += locE1; count++; }}
                if (Grid.dNy && Grid.dNz) {locE1 = IP(i,j-1,k-1).get_energy(*alpha, *beta); if (locE1 != 0.0) { sumEnergies += locE1; count++; }}
                if (Grid.dNy && Grid.dNz) {locE1 = IP(i,j+1,k-1).get_energy(*alpha, *beta); if (locE1 != 0.0) { sumEnergies += locE1; count++; }}
                if (Grid.dNy && Grid.dNz) {locE1 = IP(i,j-1,k+1).get_energy(*alpha, *beta); if (locE1 != 0.0) { sumEnergies += locE1; count++; }}
                if (Grid.dNy && Grid.dNz) {locE1 = IP(i,j+1,k+1).get_energy(*alpha, *beta); if (locE1 != 0.0) { sumEnergies += locE1; count++; }}
...
void InterfaceProperties::SetEnergyToZeroAtHalo(const PhaseField& Phase)
{
     // Set interface energies at interface halo to zero if all grains are stable
     // TODO the criterion when an interface may be exprapolated could be improved
     // it does not need to be stable for all grains, but it needs sufficiently 
     // many data points to perform the extrapolation
     auto& PF = (Grid.Resolution != Resolutions::Dual) ? Phase.Fields : Phase.FieldsDR;
     auto& IP = (Grid.Resolution != Resolutions::Dual) ? Properties : PropertiesDR;
     OMP_PARALLEL_STORAGE_LOOP_BEGIN(i,j,k,IP,IP.Bcells(),)
     if (PF(i,j,k).interface_halo())
     {
         bool grainsStable = true;
         for (auto alpha  = PF(i,j,k).cbegin(); alpha != PF(i,j,k).cend(); ++alpha)
         {
...
void InterfaceProperties::Extrapolate(const PhaseField& Phase, const BoundaryConditions& BC)
{
    ...
    switch (ExtrapolationMode)
    {
        ...
        case ExtrapolationModes::ZerothOrder:
            SetEnergyToZeroAtHalo(Phase);
            for (size_t n = 0; n <= MaxExtrapolationIterations; n++)
            {
                Complete = ZerothOrderExtrapolation(Phase);
                if (Complete) break;
            }
            break;
