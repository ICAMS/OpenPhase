...
namespace openphase
{
BenziGas::EquilibriumValues_t BenziGas::EquilibriumValues(const double InputBenziParameter)
{
    ...
    const std::vector<BenziGas::EquilibriumValues_t> EquilibriumValues =
    ...
    EquilibriumValues_t PreviousEquilibriumValue = EquilibriumValues[0];
    EquilibriumValues_t Interpolated  = {0.0,0.0,0.0,0.0};
    for (auto& EquilibriumValue: EquilibriumValues)
    {
        if (EquilibriumValue.BenziParameter == InputBenziParameter)
        {
            return EquilibriumValue;
        }
        else if ((PreviousEquilibriumValue.BenziParameter - InputBenziParameter       > 0.0) and
                 (                    InputBenziParameter - EquilibriumValue.BenziParameter > 0.0))
        {
            Interpolated = PreviousEquilibriumValue;

            const double dT = (EquilibriumValue.BenziParameter - PreviousEquilibriumValue.BenziParameter);
            const double DT = (InputBenziParameter             - PreviousEquilibriumValue.BenziParameter);
            const double A  = DT/dT;

            Interpolated.BenziParameter    = InputBenziParameter;
            Interpolated.lbVaporDensity   += (EquilibriumValue.lbVaporDensity   - PreviousEquilibriumValue.lbVaporDensity)*A;
            Interpolated.lbLiquidDensity  += (EquilibriumValue.lbLiquidDensity  - PreviousEquilibriumValue.lbLiquidDensity)*A;
            Interpolated.lbSurfaceTension += (EquilibriumValue.lbSurfaceTension - PreviousEquilibriumValue.lbSurfaceTension)*A;

            return Interpolated;
        }
        PreviousEquilibriumValue = EquilibriumValue;
    }
    if (Interpolated.BenziParameter   == 0.0)
    if (Interpolated.lbVaporDensity   == 0.0)
    if (Interpolated.lbLiquidDensity  == 0.0)
    if (Interpolated.lbSurfaceTension == 0.0)
    {
        ConsoleOutput::WriteExit("BenziParameter outside stable interval [-4.02,-5.86]! The simulation will crash!", "BenziGas", "EquilibriumValues");
