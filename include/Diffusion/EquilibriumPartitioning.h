...
#ifndef EQUILIBRIUMPARTITIONING_H
#define EQUILIBRIUMPARTITIONING_H
...
class EquilibriumPartitioning_Impl;

class OP_EXPORTS EquilibriumPartitioning : public OPObject                      ///< Composition partitioning solver
{
 public:
    EquilibriumPartitioning();
    ~EquilibriumPartitioning();

    EquilibriumPartitioning(Settings& locSettings,
    ...
    void CalculatePhaseConcentrations(PhaseField& Phi,
                                      ...
                                      BoundaryConditions& BC);                  ///< Calculates phase compositions for each present phase from total composition
 protected:
 private:
    std::unique_ptr<EquilibriumPartitioning_Impl> impl_;
};
