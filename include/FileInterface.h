...
class OP_EXPORTS FileInterface                                                  ///< Input file interface class
{
 public:
    ...
    static Quaternion ReadParameterQ(const std::stringstream& Inp,              ///< Read quaternion.
        int currentLocation,
        const std::string Key,
        const bool mandatory = true,
        const Quaternion defaultval = Quaternion());

    static EulerAngles ReadParameterEA(const std::stringstream& Inp,            ///< Read Euler angles.
        int currentLocation,
        const std::string Key,
        const bool mandatory = true,
        const EulerAngles defaultval = EulerAngles());

    static dVectorN ReadParameterAA(const std::stringstream& Inp,               ///< Read Axis-angle rotation representation.
        int currentLocation,
        const std::string Key,
        const bool mandatory = true,
        const dVectorN defaultval = {1.0,1.0,1.0,0.0});
