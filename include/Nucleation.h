...
enum class NucleiGenerationModes
{
    ...
    Continuous,                                                                 ///< Nucleation seeds are generated continuously making sure that there is always constant number of active seeds
...
class Nucleus                                                                   ///< Nucleus properties
{
 public:
    ...
    void read(std::fstream& inp)
    {
        ...
        orientation.read_ASCII(inp);
        position.read_ASCII(inp);
    ...
    void write(std::fstream& out) const
    {
        ...
        out << orientation.get_output_string() << std::endl;
        out << position.get_output_string() << std::endl;
...
struct NucleationParameters                                                     ///< Pairwise nucleation settings ("phase alpha" in "phase beta")
{
    ...
    NucleiGenerationModes GenerationMode;                                       ///< Nuclei generation mode
    ...
    NucleationParameters():
        ...
        SeedRadiusMIN(1.0),
        SeedRadiusMAX(1.0),
        GenerationMode(NucleiGenerationModes::Static),
...
class OP_EXPORTS Nucleation : public OPObject                                   ///< Handles the nucleation of new phases and grains
{
  ...
  private:
    ...
    Table< NucleationParameters > Parameters;                                  ///< Stores nucleation parameters for each pair of phases (A in B)
